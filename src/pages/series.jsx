...
export default function seriePage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/tv?api_key=72eaf7b03201f089b626e3c6c35a2eed&sort_by=${selectedSortOption}&with_genres=${selectedGenreOptions}&first_air_date_year=${startYear}&include_adult=${adult}&page=${currentPage}`
      );
      const data = await response.json();
  ...
  const handleSearchBar = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/search/tv?api_key=72eaf7b03201f089b626e3c6c35a2eed&query=${encodeURIComponent(
          searchText
        )}&page=${currentPage}`
      );
      const data = await response.json();
  ...
  const handlePaginationChange = async (newPage) => {
    ...
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/tv?api_key=72eaf7b03201f089b626e3c6c35a2eed&sort_by=${selectedSortOption}&with_genres=${selectedGenreOptions}&first_air_date_year=${startYear}&include_adult=${adult}&page=${newPage}`
      );
      const data = await response.json();
      setSearchResults(data.results);
      setTotalPages(data.total_pages);
          ...
          <Col fluid>
            ...
            <Grid.Container gap={2} justify="center" width="fit-content">
              {searchResults.map((item) => (
                <Grid xs={12} sm={12} key={item.id} className="shadow-md" direction='row' css={{ background: "#0C1E3C", marginBottom: "10px", borderRadius: "15px" }}>
